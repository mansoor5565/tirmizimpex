                        ...
                        <table class="table datatable">
                            ...
                            <tbody>
                                @foreach ($leathers as $leather)
                                    <tr>
                                        <td>{{ $leather->type }}</td>
                                        <td>{{ implode(', ', $leather->leatherColors->pluck('color')->toArray()) }}</td>
                                        <td>{{ $leather->created_at->diffForHumans() }}</td>
                                        <td>
                                            <a href="/leather/show/{{$leather->id}}" class="action-btn btn btn-success mr-2">
                                                <i class="bi bi-eye"></i> <span>View</span>
                                            </a>
                                            <a href="/leather/edit/{{ $leather->id }}"
                                                class="action-btn btn btn-warning mr-2 text-white">
                                                <i class="bi bi-pencil-square"></i> <span>Edit</span>
                                            </a>
                                            <a href="/leather/delete/{{ $leather->id }}"
